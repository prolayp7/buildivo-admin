...
async function proxy(request: NextRequest, context: Context, method: "GET" | "PATCH" | "DELETE") {
  const token = request.cookies.get(ACCESS_TOKEN_COOKIE)?.value;
  if (!token) return NextResponse.json({ message: "Your session has expired. Sign in again." }, { status: 401 });
  const { id } = await context.params;
  if (!/^\d+$/.test(id)) return NextResponse.json({ message: "Invalid bundle ID." }, { status: 400 });
  try {
    const response = await fetch(getAdminApiUrl(`admin/bundles/${id}`), {
      method,
