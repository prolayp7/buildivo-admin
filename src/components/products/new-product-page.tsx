...
type ProductDetail = {
  ...
  categoryId: number; brandId: number | null; toolPlatform: string | null; productConditionId: number | null; taxRateId: number | null; status: ProductStatus;
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setBrandId(product.brandId ? String(product.brandId) : ""); setToolPlatform(product.toolPlatform ?? ""); setProductConditionId(product.productConditionId ? String(product.productConditionId) : "");
    ...
    try {
      const response = await fetch(productId ? `/api/products/${productId}` : "/api/products", {
        ...
        body: JSON.stringify({
          ...
          ...(brandId ? { brandId: Number(brandId) } : {}), ...(toolPlatform.trim() ? { toolPlatform: toolPlatform.trim() } : {}), secondaryCategoryIds, relatedProductIds: relatedProducts.map((item) => item.id), ...(summary.trim() ? { shortDescription: summary.trim() } : {}),
